...
function logAudit(action, details = {}) {
    try {
        const store = typeof AppStore !== 'undefined' ? AppStore : null;
        const entry = {
            id: generateId(),
            timestamp: new Date().toISOString(),
            user_id: store?.currentUser?.id || 'system',
            user_name: store?.currentUser?.name || 'System',
            user_role: store?.currentUser?.role || 'system',
            action,
