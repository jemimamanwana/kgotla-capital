...
function renderDashboard(el) {
    ...
    el.innerHTML = `
        <div class="section-header"><h2>Dashboard Overview</h2><span style="color:var(--text-muted);font-size:0.85rem">${formatDate(new Date())}</span></div>
        <div class="stats-grid">
            ...
            <div class="stat-card accent"><div class="stat-label">Active SMEs</div><div class="stat-value">${activeSMEs}</div><div class="stat-change up">↑ 2 new this month</div></div>
        ...
        <div class="grid-2">
            <div class="card"><div class="card-header"><h4>Capital Deployment</h4></div><div class="card-body"><div class="chart-container"><canvas id="chart-capital"></canvas></div></div></div>
        ...
        <div style="margin-top:20px" class="grid-2">
            <div class="card"><div class="card-header"><h4>Recent Activity</h4></div><div class="card-body">
                <div class="timeline">${AppStore.payments.slice(0,5).map(p => `
                    <div class="timeline-item"><div class="tl-time">${formatDate(p.date)}</div><div class="tl-title">${p.type === 'disbursement' ? 'Disbursement' : p.type === 'repayment' ? 'Repayment' : 'Investor Payout'} - ${p.reference}</div><div class="tl-desc">${formatPula(p.amount)} · <span class="badge-status ${p.status === 'completed' ? 'active' : p.status === 'late' ? 'high' : 'pending'}">${p.status}</span></div></div>
                `).join('')}</div>
            </div></div>
            <div class="card"><div class="card-header"><h4>Pool Performance</h4></div><div class="card-body">
                ${AppStore.pools.map(pool => {
                    ...
                    return `<div style="margin-bottom:20px"><div style="display:flex;justify-content:space-between;margin-bottom:8px"><strong style="font-size:0.9rem">${pool.name}</strong><span style="font-size:0.8rem;color:var(--text-muted)">${formatPercent(pct)} subscribed</span></div><div class="progress-bar"><div class="fill green" style="width:${pct}%"></div></div><div class="tranche-visual" style="margin-top:8px">${tranches.map(t => `<div class="tranche-bar ${t.type}" style="width:${t.allocation_pct}%">${t.type.charAt(0).toUpperCase()} ${t.allocation_pct}%</div>`).join('')}</div></div>`;
                }).join('')}
            </div></div>
        </div>
    `;
    ...
    chartInstances.capital = new Chart(document.getElementById('chart-capital'), {
        ...
        data: { labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Disbursements', data: [0,0,0,1250000,300000,0], backgroundColor: '#0D6E4F', borderRadius: 6 },
            { label: 'Repayments', data: [0,0,0,0,107000,107000], backgroundColor: '#C4922A', borderRadius: 6 }
        ]},
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } }, scales: { y: { beginAtZero: true, ticks: { callback: v => 'P' + (v/1000) + 'k' } } } }
    });
    ...
    chartInstances.sectors = new Chart(document.getElementById('chart-sectors'), {
        type: 'doughnut',
        data: { labels: Object.keys(sectors), datasets: [{ data: Object.values(sectors), backgroundColor: ['#0D6E4F','#14A073','#C4922A','#E5B94E','#2563EB','#7C3AED'], borderWidth: 0 }] },
